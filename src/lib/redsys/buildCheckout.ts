...
export type RedsysCheckoutInput = {
  ...
  origin: string;
  amount: string;
  /** Pedido interno de la aplicación, usado para volver a /ok y /ko. */
  orderId: string;
  /** Número único enviado a Redsys como DS_MERCHANT_ORDER. */
...
export function buildRedsysCheckout(input: RedsysCheckoutInput): RedsysCheckoutResult {
  const { customerId, origin, amount, orderId, redsysOrderId = orderId, merchantSecretB64, tpvUrl } = input;
