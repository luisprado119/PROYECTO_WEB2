          ...
          <div className="space-y-md">
            {cestaItems.map((item, i) => (
              ...
              >
                ...
                <Link
                  href={`/product/${item.productId}?cantidad=${item.cantidad}`}
                  className="text-label-md text-md-primary hover:underline flex-shrink-0 hidden sm:block ml-sm"
                >
                  Editar
                </Link>
              </div>
            ...
            {!isLoggedIn ? (
              <div className="space-y-md">
                <div className="p-md bg-amber-50 border border-amber-200 rounded-lg">
                  <p className="text-label-md font-bold text-amber-800">⚠️ Inicia sesión para continuar</p>
                  <p className="text-[12px] text-amber-700 mt-1">Necesitas una cuenta para confirmar el pedido y pagar.</p>
                </div>
                <Link
                  href="/login"
